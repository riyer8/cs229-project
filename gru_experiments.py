from models.gru_predictor import GRUVolatilityPredictor
...
def feature_tuning():
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        for feature_set in FEATURES:
            ...
            predictor = GRUVolatilityPredictor(sequence_length=30)
...
# feature_tuning()
# Best Feature Combo: ['Daily Return', 'High-Close', 'Low-Open', 'Close Change'] with MAE 2.299304485321045
...
def hidden_size_tuning():
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        for dim in hidden_dims:
            ...
            predictor = GRUVolatilityPredictor(sequence_length=30, hidden_dim=dim)
            train_loader, val_loader, test_loader, dates_val_seq, y_val_seq, dates_test_seq, y_test_seq = predictor.prepare_data(
                data=data,
...
# hidden_size_tuning()
# Best Hidden Dim: 32 with MAE 2.412525177001953
hidden_dim = 32

def run_test():
    predictor = GRUVolatilityPredictor(sequence_length=30, hidden_dim=hidden_dim)
    train_loader, val_loader, test_loader, dates_val_seq, y_val_seq, dates_test_seq, y_test_seq = predictor.prepare_data(
        data=data,
        features=best_feature,
        target='Volatility'
