...
class GRUVolatilityPredictor:
    ...
    def build_model(self, input_dim, output_dim=1):
        ...
        self.model = GRURegressor(input_dim=input_dim, hidden_dim=self.hidden_dim, output_dim=output_dim).to(self.device)
    ...
    def plot_loss(self, save_path="gru_loss.png"):
        plt.figure(figsize=(10, 5))
        plt.plot(range(1, self.epochs + 1), self.train_losses, label='Training Loss')
        plt.xlabel('Epoch')
        plt.ylabel('MSE Loss')
        plt.title('GRU Training Loss on Dow Jones Data Over Epochs')
        plt.legend()
        if save_path:
    ...
    def plot_predictions(self, dates, y_true, y_pred, horizon=HORIZON, save_path="gru_preds.png"):
        ...
        plt.title(f"GRU - Predicted vs. Actual Volatility on Dow Jones Test Data (Horizon: {horizon} Days)")
        plt.legend()
